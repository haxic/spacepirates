...
public class ShadowBox {
	...
	protected void update() {
		...
		Vector4f rotationForward = rotation.transform(FORWARD, new Vector4f());
		Vector3f forwardVector = new Vector3f(rotationForward.x, rotationForward.y, rotationForward.z);
	...
	protected Vector3f getCenter() {
		...
		Vector4f invertedLightTransformed = invertedLight.transform(cen, new Vector4f());
		return new Vector3f(invertedLightTransformed.x, invertedLightTransformed.y, invertedLightTransformed.z);
	}
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector, Vector3f centerNear, Vector3f centerFar) {
		Vector4f rotationTransformed = rotation.transform(UP, new Vector4f());
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction, float width) {
		...
		Vector4f lightViewMatrixTransformed = lightViewMatrix.transform(point4f,  new Vector4f());
		return lightViewMatrixTransformed;
	}
	...
	private float getAspectRatio() {
		return (float) DisplayManager.getAspectRatio();
	}
